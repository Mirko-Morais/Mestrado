...
def validaModeloComProbabilidade(df, X_cols, Y_col, Modelo, Scaler, Metrica):
  X = Scaler.fit_transform(df[X_cols])
  y = df[Y_col].values
  y_predict_proba = Modelo.predict_proba(X)
  y_predict = Modelo.predict(X)
  return y_predict_proba, y_predict, Metrica(y, y_predict) 